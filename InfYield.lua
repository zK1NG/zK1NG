if IY_LOADED and not _G.IY_DEBUG == true then
	error("MalwareHub is already running!",0)
	return
end
...
if not game:IsLoaded() then
	...
	notLoaded.Text = 'MalwareHub is waiting for the game to load'
	game.Loaded:Wait()
	notLoaded:Destroy()
end
...
About.TextSize = 14
About.Text = "Plugins are .MH files and should be located in the 'workspace' folder of your exploit."
...
function saves()
	if writefileExploit() then
		if pcall(function() readfile("MalwareHub." .. fileExtension) end) then
			if readfile("MalwareHub." .. fileExtension) ~= nil then
				local success, response = pcall(function()
					local json = game:GetService("HttpService"):JSONDecode(readfile("MalwareHub." .. fileExtension))
				...
				if not success then
					...
					writefileCooldown("MalwareHub." .. fileExtension, defaults)
					wait()
					saves()
				end
			else
...
function updatesaves()
	if nosaves == false and writefileExploit() then
		...
		writefileCooldown("MalwareHub." .. fileExtension, game:GetService("HttpService"):JSONEncode(update))
	end
end
...
CMDs[#CMDs + 1] = {NAME = 'attach', DESC = 'MalwareHub'}
...
function addPlugin(name)
	if name:lower() == 'plugin file name' or name:lower() == 'MalwareHub.' .. fileExtension or name == 'MalwareHub' then
